...
export const section: Section = {
  ...
  cards: [
    {
      ...
      keys: 'rate counter',
      badge: 'basic',
      description: 'Per-second average rate of a counter over the window. Resets are handled automatically. Use this for graphs and alerts on counters.',
    ...
    },
    {
      ...
      badge: 'advanced',
      description: 'Rate based on the <em>last two</em> samples only — reactive but spiky. Good for short-lived dashboards, bad for alerts.',
    ...
    },
    {
      ...
      badge: 'basic',
      description: 'Total counter increase over the window. Equivalent to <code>rate × seconds</code>, but reads more naturally.',
  ...
  sum by (le, job) (
    ...
    },
    {
      ...
      badge: 'advanced',
      description: 'Linear regression — predicts the value <code>t</code> seconds from now. Classic disk-full alert.',
      code: `predict_linear(
  node_filesystem_avail_bytes[1h], 4 * 3600
    ...
    },
    {
      ...
      badge: 'basic',
      description: 'Aggregate a range vector along the <em>time</em> axis (per series). Family includes <code>avg / min / max / sum / count / quantile / stddev / last</code>.',
...
absent_over_time(up{job="api"}[5m])`,
    },
    {
      ...
      keys: 'changes resets',
      badge: 'advanced',
      description: '<code>changes</code> counts value changes (gauges); <code>resets</code> counts counter resets in the window.',
...
resets(process_start_time_seconds[1h])`,
    },
    {
      ...
      badge: 'advanced',
      description: 'Rewrite a label using a regex on another label. Most common use: aligning labels for a join.',
      code: `label_replace(
  up, "host", "$1", "instance", "(.*):.*"
)`,
    },
    {
      ...
      keys: 'label_join',
      badge: 'advanced',
      description: 'Concatenate label values into a new label using a separator.',
    ...
    },
    {
      ...
      badge: 'advanced',
      description: 'Bridge between scalar and vector worlds — useful in arithmetic against constants.',
      code: `vector(1)
scalar(sum(up))`,
    },
    {
      ...
      keys: 'time clock now',
      badge: 'advanced',
      description: 'Unix timestamp (seconds) of the evaluation. Combine with <code>process_start_time_seconds</code> for uptime.',
      code: `time() - process_start_time_seconds`,
    },
    {
      title: '<code>clamp</code> / <code>clamp_min</code> / <code>clamp_max</code>',
      titleHtml: true,
      keys: 'clamp clamp_max clamp_min',
      badge: 'advanced',
      description: 'Squeeze values into a range. Handy for sanitizing dashboards.',
    ...
    },
    {
      ...
      keys: 'sort',
      badge: 'basic',
      description: 'Sort the result vector by sample value. Affects table view, not graphs.',
    ...
    },
    {
      ...
      badge: 'basic',
      description: 'Vectors line up by matching label sets. Use <code>on()</code>/<code>ignoring()</code> + <code>group_left/right</code> for many-to-one joins.',
